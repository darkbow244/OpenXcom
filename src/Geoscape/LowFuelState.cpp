...
LowFuelState::LowFuelState(Game *game, Craft *craft, GeoscapeState *state) : State(game), _craft(craft), _state(state)
{
	...
	_btnOk->onKeyboardPress((ActionHandler)&LowFuelState::btnOkClick, (SDL_Keycode)Options::getInt("keyCancel"));
	...
	_btnOk5Secs->onKeyboardPress((ActionHandler)&LowFuelState::btnOk5SecsClick, (SDL_Keycode)Options::getInt("keyOk"));
